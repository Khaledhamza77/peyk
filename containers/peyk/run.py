#!/usr/bin/env python3
"""Merged worker entrypoint — dispatches to layout/tsr/ocr's own run.py `main()` based on
--stage, the same pattern peyk-surya's own run.py already uses for its three roles (see
that container). Each stage's backends/ package and run.py under stages/<stage>/ are copied in
unmodified from their original single-purpose images (containers/peyk-layout,
containers/peyk-tsr, containers/peyk-simple-ocr) — this file only routes argv to the right one,
no stage behavior changes here.
...
--visualize, --watch, --poll-interval) reaches the stage's own unmodified argparse exactly as
before.
"""
import argparse
import runpy
